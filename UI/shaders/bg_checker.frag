//https://www.shadertoy.com/view/wdcfRs
...
void main(){
    ...
    float tf = fract(t);
    vec2 uv = 0.5 + (gl_FragCoord.xy / iResolution.xy - 0.5) * vec2(iResolution.x / iResolution.y, 1.0);
    ...
    vignette *= vignette;
    gl_FragColor = vec4(vec3(pow(clamp(sum, 0.0, 1.0) * vignette, 0.5)), 1.0);
}